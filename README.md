...
The runtime complexity of this permutation sort is $O(n * n!)$. The reason being, all permutations are calculated in a factorial order. you test the $n^{th}$ permutation, then the $(n-1)$ permutation, then the $(n-2)$ permutation, so on. It follows that $n! = n * (n-1)!$. Each iteration, I am checking if the array is sorted with my isSorted() function. This runs in $O(n)$ time, for $n$ elements in the array. This is my reasoning for concluding a runtime complexity of $O(n * n!)$.
...
## Sources:
This was a pretty difficult exercise for me, I think I have done everything correctly now. I used ChatGPT to help me figure out my general program structure, and the roles of the functions I need to write.

I read through this website: https://www.geeksforgeeks.org/bogosort-permutation-sort/
